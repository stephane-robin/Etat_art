...
Un package permet de fournir une structure hiérarchisée du programme, pour en faciliter la compréhension et l'utilisation. Un \index{package}package standard est ainsi un ensemble de modules Python, de modules écrits en C, de métadonnées et de bases de données.

Il comporte traditionnellement un fichier \textbf{\_\_init\_\_.py} dans chaque répertoire, même si ce répertoire est vide. En particulier, le répertoire racine du package contient un fichier \textbf{\_\_init\_\_.py} qui peut définir la version du projet et le nom des modules à importer. Il est toutefois acceptable et parfois même recommandé de conserver ce fichier \textbf{\_\_init\_\_.py} vide.

Dans le cas d'un package exécutable, il est essentiel que celui-ci contienne un module de lancement du programme appelé  \textbf{\_\_main\_\_.py}.

Ainsi, lorsqu'un utilisateur installe un package, celui-ci présente la structure suivante 

\includegraphics[width=4.5cm]{mon_package1.png}

Du point de vue du développeur, il est nécessaire de rajouter des fichiers comportant les métadonnées du package.
...
\includegraphics[width=5.5cm]{mon_package2.png}
...
\begin{itemize}
\item le répertoire \textbf{dist} contient l'archive des sources du package \textbf{mon\_package-0.1.0.tar.gz} (ici la version 0.1.0), construite en compressant le répertoire \textbf{mon\_package}. Il contient également la distribution compilée \textbf{mon\_package-0.1.0-py3-none-any.whl} permettant de décompresser l'archive du format \textit{.tar.gz} et de rendre le package exécutable. Lors de l'installation du package par l'utilisateur, c'est une copie du fichier \textbf{mon\_package-0.1.0.tar.gz} qui est téléchargée, stockée et exécutable sous le nom de \textbf{mon\_package} à l'emplacement prévu des packages dans l'ordinateur de l'utilisateur.
...
\item le répertoire \textbf{mon\_package.egg-info} contient les ressources et les métadonnées du projet nécessaires à l'installation du package. Le format \textit{.egg} facilite la désinstallation et la mise à niveau du code, car le projet est essentiellement autonome dans un unique répertoire. Il autorise aussi l'installation de plusieurs versions d'un même projet simultanément. Il s'agit d'un format adapté à la distribution de plugins pour des applications extensibles et des frameworks, similaire au format \textit{.jar} en Java. Il permet également de distribuer les gros packages en entités séparées. Il est fréquent que \textit{pip} ne parvienne pas à installer les fichiers au format \textit{.egg} qui ne sont pas standardisés et ne tiennent pas compte des différents interpréteurs CPython, jython, pypy, ... 
...
\item le fichier \textbf{pyproject.toml} précise les métadonnées du package et sert à la construction de celui-ci. \textbf{pyproject.toml} définit automatiquement le fichier \textbf{setup.py} qui, dans le cas de \textit{Poetry}, n'est qu'un fichier temporaire servant à la construction de  \textbf{mon\_package.egg-info}. Le fichier \textbf{pyproject.toml} remplace donc les fichiers \textbf{setup.cfg},\textbf{MANIFEST.in} et incorpore les différents formats \textbf{build.sh}, \textbf{meta.yaml}, \textbf{bld.bat}.  Il gère également l'installation des dépendances et remplace donc le fichier \textbf{requirements.txt}. 
...
Une fois que le package est publié dans l'index PyPI, sa distribution est en fait une copie du dossier \textbf{dist} et on peut télécharger à partir de PyPI les deux fichiers \textbf{mon\_package-0.1.0.tar.gz} et \textbf{mon\_package-0.1.0-py3-none-any.whl}.

Du point de vue de l'utilisateur, lorsqu'il télécharge l'archive du package à partir de PyPI, sa structure est la suivante 

\includegraphics[width=4.5cm]{mon_package_poetry_archive.png}

On remarque que le dossier \textbf{dist} relatif à la création de la distribution a bien évidemment disparu, ainsi que le dossier de test \textbf{tests} et le fichier \textbf{poetry.lock} qui a servi à fixer la version des dépendances. Seul le fichier \textbf{PKG-INFO} a été conservé du répertoire \textbf{mon\_package.egg-info}.

Dans ce cas-là, l'archive du package sera, en fonction des réglages par défaut, présente dans le dossier \textbf{Téléchargements} de l'ordinateur, mais ne sera pas installée. Cela signifie que l'exécution du package par l'instruction standard

\begin{verbatim}
  python -m  mon_package
\end{verbatim} 

ne fonctionnera pas car le système n'est pas en mesure de trouver le package à l'emplacement dédié des packages.
...
sans le "-m" ne fonctionnera pas non plus, car l'interpréteur qui ne va pas considérer \texttt{  mon\_package} comme un package et ne va pas chercher le fichier \textbf{\_\_main\_\_.py} pour l'exécuter.

En revanche, si on se plaçant à l'intérieur de l'archive, l'utilisation de l'instruction

\begin{verbatim}
  python __main__.py
\end{verbatim}

exécutera le package en tant que programme. Cela n'a que peu d'intérêt vu qu'on attend d'un package qu'il s'exécute simplement sans qu'on ait à chercher préalablement son emplacement.

On remarque donc qu'il est nécessaire pour l'utilisateur d'installer le package sur son ordinateur à l'emplacement réservé pour les packages afin de pouvoir l'exécuter. C'est le rôle de la distribution compilée \textbf{mon\_package-0.1.0-py3-none-any.whl} qui va s'exécuter lors de l'instruction

\begin{verbatim}
  pip install mon_package
\end{verbatim}  

On peut dès lors entrevoir une confusion possible pour définir la racine du package. Aux yeux de l'utilisateur comme du développeur, la racine peut se trouver au niveau du module \textbf{\_\_main\_\_.py} si on installe le package à partir de PyPI, ou au niveau du fichier \textbf{pyproject.toml} si on télécharge le package à partir de PyPI. Nous considèrerons que la racine du package se trouve au niveau du fichier \textbf{pyproject.py}, mais tous les chemins d'accès auront pour référence le module d'exécution primaire \textbf{\_\_main\_\_.py}.
...
Lorsque l'utilisateur installe le package avec l'instruction \texttt{  pip install mon\_package} et qu'il se déplace dans le dossier du package à l'emplacement prévu pour les packages, il constate la structure suivante
...
\item \textbf{se fait} à partir du répertoire courant, quel qu'il soit, avec l'instruction \texttt{  python -m crisprbuilder\_tb \textemdash \textemdash collect ERR2704808}. C'est à l'ordinateur d'aller chercher le package à son emplacement. 
\end{itemize}
...
Avant de créer le package, il faut commencer par construire un programme qui fonctionne localement sur une machine correctement paramétrée. Pour cela, nous définissons un \index{environnement de développement}environnement de développement, qui nous sert à exécuter le code et à le tester au fur et à mesure de son élaboration. Cet environnement contient tous les modules et packages nécessaires à son bon fonctionnement, c'est à dire les dépendances du projet.
...
Dans le cadre de notre étude, nous créons un répertoire \textbf{biologie}, nous nous plaçons dedans puis nous créons, à l'aide de \textit{venv}, un environnement de développement \textbf{env\_bio} avec l'instruction suivante
...
\item \textit{pip} pour l'installation de distributions à partir de PyPI, en tenant compte des dépendances. Certains packages sont référencés sur l'index sans que les distributions ne soient disponibles sur PyPI, et il est possible de télécharger les distributions à partir d'URL définies dans un fichier \textbf{setup.py}.
...
WWWWW
 ...
 Notons que la version 3.3 de Python a introduit la notion de \textit{Implicit Namespace Packages}, qui permet la création de packages sans la présence de fichiers  \textbf{\_\_init\_\_.py}. Nous choisissons d'insérer des fichiers \textbf{\_\_init\_\_.py} vides dans CRISPRbuilder\_TB, car nous verrons qu'ils sont créés automatiquement lors de la construction du package..
...
\begin{itemize}
\item le répertoire \textbf{crisprbuilder\_tb} contient initialement le fichier \textbf{\_\_init\_\_.py} et c'est dans ce répertoire que nous allons rajouter tous les composants principaux du code, à savoir  \textbf{\_\_main\_\_.py}, \textbf{fonctions.py}, \textbf{bdd.py} et les répertoires \textbf{data}, \textbf{doc}, \textbf{REP} et \textbf{tmp} avec leurs contenus. En effet, la création de l'archive va compresser ce répertoire \textbf{crisprbuilder\_tb} afin de le publier. Il devra contenir tout ce qui est nécessaire au fonctionnement de l'application. Lors de sa création, le fichier\textbf{\_\_init\_\_.py} contenait uniquement la version du package. Pour éviter tout conflit de version avec le fichier \textbf{pyproject.toml}, nous vidons le fichier \textbf{\_\_init\_\_.py} du répertoire \textbf{crisprbuilder\_tb}. Nous rajoutons également une \textbf{LICENSE} qui pourra être consultée par l'utilisateur.
\item le répertoire \textbf{tests} valide la bonne configuration du package. Nous précisons dans le fichier \textbf{\_\_init\_\_.py} le bon numéro de version
\item le fichier \textbf{README.rst} fournit une brève description du package, et nous le modifions au format markdown \textbf{README.md} pour des raisons de commodité. Ce fichier restera au niveau de la racine du package afin de compléter la rubrique "Description" de PyPI au moment de la publication. Il sera quand même présent dans l'archive si l'utilisateur la télécharge. 
\item le fichier \textbf{pyproject.toml} précise les métadonnées du package telles que son nom, sa version, sa description, l’emplacement de son dépôt (par exemple sur GitHub), l’adresse email de l’auteur du package, et la version des dépendances.
\item nous rajoutons à la racine du package un fichier \textbf{.gitignore} pour la gestion des versions avec \textit{Git}. 
\end{itemize}
...
qui assure la compatibilité de la dépendance \textit{nom\_dependance} avec le package CRISPRbuilder\_TB. Il est également possible d’imposer certaines contraintes sur les versions des dépendances ou encore de rentrer manuellement les dépendances dans le fichier \textbf{pyproject.toml}, mais l’instruction 
\texttt{  poetry add nom\_dependance} 
...
qui crée les fichiers \textbf{poetry.lock} et \textbf{setup.py}, ainsi que le répertoire \textbf{crisprbuilder\_tb.egg-info} comprenant le code, les ressources et les métadonnées du projet. Dans le cas de \textit{Poetry}, \textbf{setup.py} n'est en fait qu'un fichier temporaire servant à la construction de \textbf{crisprbuilder\_tb.egg-info}. 
...
Attention, si la distribution comporte la mention "py2" au lieu de "py3", cela signifie que la construction sous \textit{Poetry} a été réalisée à partir de Python 2. Il faut donc bien s'assurer, avant la création du package, de choisir une version de Python 3 dans le fichier \textbf{pyproject.toml} ou de travailler dans le répertoire courant avec une version 3 de Python.
...
\section{La structure de l'archive du package à télécharger}
...
Le package, une fois installé à l'emplacement des packages, présente la structure fonctionnelle suivante :
