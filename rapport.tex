...
Ainsi, une fois installé, le package présente la structure suivante pour l'utilisateur :
...
Ainsi, un package standard présente initialement la structure suivante pour le développeur :
...
Il faut également rajouter l'archive et la distribution compilée en vue de publier le package. Nous entrerons dans le détail de ce processus dans la rubrique suivante avec un package construit par \textit{Poetry}.
...
Avec la PEP-518\footnote{\index{PEP-518}\textbf{PEP-518:} specifying minimum build system requirements for Python projects}, le PyPA\footnote{\index{PyPA}\textbf{PyPA:} Python Packaging Authority} a proposé un nouveau standard au format \textit{.toml} pour créer des packages sous \textit{Poetry}. Ainsi le fichier \textbf{pyproject.toml} remplace les fichiers \textbf{setup.py}, \textbf{setup.cfg} et \textbf{MANIFEST.in}. Il gère également l'installation des dépendances et remplace donc le fichier \textbf{requirements.txt}.

Si l'utilisateur télécharge l'archive du package à partir de PyPI, sa structure sera la suivante :

\includegraphics[width=4.5cm]{mon_package_poetry_archive.png}

Dans ce cas l'archive du package sera, en fonction des réglages par défaut, présente dans le dossier \textbf{Téléchargements} de l'ordinateur, mais ne sera pas installée. Cela signifie que l'instruction
...
Finalement, en se plaçant à l'intérieur de l'archive, l'utilisation de l'instruction

\begin{verbatim}
  python __main__.py
\end{verbatim}

ne fonctionnera pas car XXX A TESTER

On remarque la présence du fichier \textbf{setup.py} qui devait être remplacé par \textbf{pyproject.toml}. En fait, le fichier \textbf{setup.py} est automatiquement construit par \textbf{pyproject.toml}, et s'occupe par la suite de la construction du package.

Une fois installé, le package construit par \textit{Poetry} présente la structure suivante pour l'utilisateur :

\includegraphics[width=4.5cm]{mon_package_poetry.png}

Notons que pour l'utilisateur, la racine du package se trouve au niveau du module \textbf{\_\_main\_\_.py}.

Du point de vue du développeur, il faut rajouter les métadonnées du package ainsi que d'autres éléments qui permettent de construire la distribution suivant un numéro de version choisi, et de tester la conformité de cette distribution.

\begin{itemize}
\item la création du répertoire portant le nom du package est automatique avec \textit{Poetry}, de même que celle des fichiers \textbf{\_\_init\_\_.py}. 
\item le répertoire \textbf{dist} créé au moment de la construction du package contient d'une part l'archive du package \textbf{mon\_package-0.1.0.tar.gz} construite à partir du répertoire \textbf{mon\_package} contenant tous les modules et bases de données nécessaire à l'exécution du programme. C'est une copie du fichier \textbf{mon\_package-0.1.0.tar.gz} qui est téléchargée et stockée dans l'ordinateur de l'utilisateur sous le nom de \textbf{mon\_package} au moment de l'installation du package. Le répertoire \textbf{dist} contient d'autre part la distribution compilée \textbf{mon\_package-0.1.0-py3-none-any.whl} est le format standard proposé par PEP. Elle permet de décompresser l'archive du format \textit{.tar.gz} et de rendre le package exécutable.
\item le répertoire \textbf{mon\_package.egg-info} contient des métadonnées nécessaires à l'installation du package. Le format \textit{.egg} développé par \textit{setuptools} est l'équivalent en Python du format \textit{.jar} en Java, mais \textit{pip} ne parvient généralement pas à installer les fichiers au format \textit{.egg} qui ne sont pas standardisés et ne tiennent pas compte des différents interpréteurs CPython, jython, pypy, ...
\item le réperoire \textbf{tests} permet de valider la conformité de la distribution dont le numéro de version apparaît dans le fichier \textbf{\_\_init\_\_.py} qu'il contient.
\item le fichier \textbf{pyproject.toml} précise les métadonnées du package et sert à la construction de celui-ci. Les différents formats \textbf{build.sh}, \textbf{meta.yaml} et \textbf{bld.bat}, sont également incorporés dans le fichier \textbf{pyproject.toml}. 
\item le fichier \textbf{poetry.lock} empêche les dépendances de télécharger la dernière version au moment de l'installation, en fixant la version utilisable par le package.
\end{itemize}

Ainsi, le package construit par \textit{Poetry} présente la structure suivante pour le développeur :

\includegraphics[width=9cm]{mon_package_poetry_complet.png}

Notons que pour le développeur, la racine du package se trouve au niveau du fichier \textbf{pyproject.toml}, ce qui est important afin que les métadonnées du package puissent être interprétées.

Une fois que le package est publié dans l'index PyPI, sa distribution est en fait une copie du dossier \textbf{dist} et on peut télécharger à partir de PyPI les deux fichiers :
\begin{itemize}
\item une archive contenant les sources \textbf{mon\_package-0.1.0.tar.gz}.
\item une distribution compilée \textbf{mon\_package-0.1.0-py3-none-any.whl}.
\end{itemize}
...
\item le fichier \textbf{pyproject.toml} remplace les anciens standards de définition de packages : \textbf{setup.exe} et \textbf{requirements.txt}. Il précise notamment les métadonnées du package telles que son nom, sa version, sa description, l’emplacement de son dépôt (par exemple sur GitHub), l’adresse email de l’auteur du package, et la version des dépendances. Le fichier \textbf{pyproject.toml} doit obligatoirement être placé à la racine du package au moment de sa création.
