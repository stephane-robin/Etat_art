...
Le packaging ou distribution de programmes pose traditionnellement plus de problèmes en Python que dans d'autres langages. La documentation sur Internet est souvent obsolète et entraîne de nombreuses incompréhensions. Toutefois, des sites tels que \url{https://realpython.com/python-modules-packages/}, \url{https://realpython.com/pypi-publish-python-package/}, \url{https://packaging.python.org/overview/} ou \url{https://python-poetry.org/} permettent de mieux appréhender les difficultés.
...
Un \index{package}package standard est un ensemble de modules Python, de modules en C, de métadonnées et de bases de données. Il comporte obligatoirement un fichier \textbf{\_\_init\_\_.py} dans chaque répertoire, même dans le cas d'un répertoire vide. En particulier, le répertoire racine du package contient un fichier \textbf{\_\_init\_\_.py} qui va définir la version du projet et le nom du module de lancement du programme \textbf{\_\_main\_\_.py}. Il est toutefois acceptable et parfois même recommandé de conserver ce fichier \textbf{\_\_init\_\_.py} vide. Nous avons ainsi choisi d'insérer des fichiers \textbf{\_\_init\_\_.py} vides dans CRISPRbuilder\_TB.
...
Avec la PEP-518\footnote{\index{PEP-518}\textbf{PEP-518:} specifying minimum build system requirements for Python projects}, le PyPA\footnote{\index{PyPA}\textbf{PyPA:} Python Packaging Authority} a proposé un nouveau standard au format \textit{.toml}, qui remplace les fichiers \textbf{setup.py}, \textbf{requirements.txt}, \textbf{setup.cfg} et \textbf{MANIFEST.in}. C'est ce nouveau standard qui est utilisé lors de la création d'un package avec \textit{Poetry} dans le fichier \textbf{pyproject.toml}.
...
Le format \textit{.egg} en Python est l'équivalent du format \textit{.jar} en Java. \textit{pip} ne parvient généralement pas à installer les fichiers au format \textit{egg} qui ne sont pas standard et ne tiennent pas compte des différents interpréteurs CPython, jython, pypy, ...
...
sans le "-m", car dans ce cas l'exécution entraînerait la levée d'une exception due aux différents chemins d'accès qui seraient erronés.
